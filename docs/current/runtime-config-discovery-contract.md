...
`NetchRuntimeBootstrap.InitializeProtectedAsync(...)` completes before the production pipe server accepts requests. It authenticates and decrypts the packaged `runtime-settings.nkps` in memory, parses it with the existing Netch `Setting` and server-converter semantics, and loads mode files into `Global.Modes`. Production never reads or falls back to `data/settings.json`; `InitializeAsync(...)` remains only for explicit non-production integration fixtures. The host then creates one `NetchProcessModeConfigurationCatalog` snapshot and passes the same instance to discovery, validation, and `NetchProcessModeSessionResolver`. The snapshot deep-clones the complete legacy `Setting` object plus selected server and mode execution objects; the session restores those frozen settings immediately before `MainController.StartAsync`, because legacy controllers still read runtime options from `Global.Settings`. The host does not reload configuration during its lifetime.
